...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(config, "WRITABLE_ROOTS", [target])
...
def test_copy_refuses_when_target_not_writable(env, monkeypatch):
    source, target = env["source"], env["target"]
    (source / "a.bin").write_bytes(b"payload")
    monkeypatch.setattr(config, "WRITABLE_ROOTS", [])  # nothing allowlisted
