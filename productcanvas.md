...
- The app's own SQLite DB/staging folder can end up double-visible if nested inside a whole-volume mount (e.g. `/volume1/docker/dedupe/config` also appearing, read-only, under the `/volume1` browse root). Recommended mitigation: house them under a dot-prefixed folder name (`.dedupe`) so `app/browse.py`'s existing dotfile filter hides them from the picker — documented in `docker-compose.yml`'s comments, not yet confirmed as the user's actual folder layout.
- A drive/volume attached to the NAS *after* `docker compose up` needs a container restart to become visible (Docker only sees mounts present at container start) — not fixable purely in-app; `bind-propagation: rslave` on the relevant compose mount is a possible future improvement if live (no-restart) attach turns out to matter in practice.
